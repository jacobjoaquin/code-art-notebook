...
void setup() {
  ...
  for (int i = 0; i < (int) nLines; i++) {
    PVector v = getVCoordinates(width / 2, height / 2, 64, (float) i / nLines * TWO_PI * 2);
    lineSegments.add(new LineSegment(v.x, v.y, 64, (float) i / nLines * TWO_PI));
  }
}
...
void draw() {
  ...
  for (int i = 0; i < lsSize; i++) {
    float scale = 120.0;
    float xOffset = scale * (sin(((float) i / (float) lsSize + phasor) * TWO_PI));
    float yOffset = scale * (cos(((float) i / (float) lsSize + phasor) * TWO_PI));
