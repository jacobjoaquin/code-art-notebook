...
class AsymRing {
  ...
  void draw() {
    for (Slice s : slices) {
      ...
      colorMode(HSB);
      fill(255 * d * 0.125, 255, 128 * d + 127);
      fill(255);
      stroke(0, d * 255);
      noStroke();
...
void setup() {
  ...
  while (d < (width / 2.0 * sqrt(2))) {
    ...
    ar.spacer = PI / 8.0;
    ar.minWidth = PI / 32.0 - ar.spacer;
    ar.maxWidth = PI / 3.0 - ar.spacer;
    ar.offset = offset;
    offset += PI / 3.5;    
    ar.update();
    aRings.add(ar);
...
void draw() {
  ...
  for (AsymRing ar : aRings) {
    float thisPhase = sin((phase + (float(counter++) / 50.0)) * TWO_PI);
    pushMatrix();
    rotate(thisPhase * TWO_PI / 16.0);
    ar.draw();
    popMatrix();
  }
