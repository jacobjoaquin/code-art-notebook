int nLinesPerFrame = 256;
float minLineSize = 4.0;
color bg = color(240);
color fg = color(32);
float pMin = 4.0;
float pMax = 12.0;
...
void draw() {
  ...
  for (int oy = 0; oy < nTiles; oy++) {
    nSides = 3 + oy;

    for (int ox = 0; ox < nTiles; ox++) {
      reset();
      nAngles = 3 + ox;
      pushMatrix();
      translate(ox * width / nTiles, oy * height / nTiles);
      scale(1.0 / float(nTiles));
